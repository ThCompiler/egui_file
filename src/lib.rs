...
impl FileDialog {
    ...
    fn title(&self) -> &str {
        match self.dialog_type {
            DialogType::SelectFolder => "📁  Выбрать директорию",
            DialogType::OpenFile => "📂  Открыть файл",
            DialogType::SaveFile => "💾  Сохранить файл",
        }
    }
    ...
    fn ui_in_window(&mut self, ui: &mut Ui) {
        ...
        egui::TopBottomPanel::bottom("egui_file_bottom").show_inside(ui, |ui| {
            ...
            ui.horizontal(|ui| {
                ui.label("Файл:");
                ui.with_layout(Layout::right_to_left(egui::Align::Center), |ui| {
                    if self.new_folder && ui.button("Новая папка").clicked() {
            ...
            ui.horizontal(|ui| {
                ...
                'fst: {
                    if response.changed() {
                        ...
                        if depth != self.completion.folder_depth || file_name.is_empty() {
                            self.update_completion(current_path);
                        }
                ...
                #[cfg(unix)]
                ui.with_layout(Layout::right_to_left(egui::Align::Center), |ui| {
                    ui.checkbox(&mut self.show_hidden, "Показать скрытые");
                });
            });
        });
    ...
    #[cfg(windows)]
    fn update_completion(&mut self, current_path: &Path) {
